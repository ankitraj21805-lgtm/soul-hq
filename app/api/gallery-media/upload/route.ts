...
function safe(v:string){
  return encodeURIComponent(String(v || 'unknown').trim().slice(0,70).replace(/\s+/g,'-'));
}
async function allowed(){
  const c = await cookies();
  return c.get('soul_member')?.value === '1' || c.get('soul_admin')?.value === '1';
}
async function ensure(kind:'image'|'clip'){
  ...
  if(found.error){
    await s.storage.createBucket(bucket, { public: true, fileSizeLimit: kind === 'clip' ? 100*1024*1024 : 10*1024*1024 });
  }
  return { s, bucket };
}
